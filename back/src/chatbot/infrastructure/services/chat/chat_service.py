...
from back.src.embeddings.infrastructure.database.repository.embedding_repository import SQLAlchemyEmbeddingRepository
from back.src.shared.infrastructure.database import get_session
...
def process_user_question(question: str, embeddings: List[List[float]], kmeans_labels: List[int], top_k: int = 5):
    ...
    if question_embedding:
        ...
        relevant_cluster = find_most_relevant_cluster(question_embedding)
        ...
        if relevant_cluster is not None:
            ...
            top_k_chunks = select_top_k_chunks(cluster_indices, embeddings, question_embedding, top_k=top_k)
